...
if (!module.parent) {
   ...
   socket.on('connection', function(client) {
    ...
    /*    Insert Message to log database    */
              ...
              db.logtracker.insert(obj);

    /*    Check in MongoDB add new parameters to obj JSON Object      */
              ...
              for(i=0;i<obj.count;i++)
              {
                  collection.findOne({mac:obj.members[i].mac},function(err, results){
                    obj.members[i].username = results[0].username ;
                    obj.members[i].empid = results[0].empid ;
                    console.log(results);
                  });
